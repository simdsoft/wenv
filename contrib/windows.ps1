...
$actions.init = @{
    php     = {
        ...
        # trim suffix if possible: alpha1, beta1, rc1 ...
        
        if ($xdebug_ver -match '^(\d+\.\d+\.\d+)') {
          $xdebug_plain_ver = $matches[1]
        }
        else {
          throw "Invalid xdebug version format: $xdebug_ver"
        }
        if ([Version]$xdebug_plain_ver -ge [Version]'3.4.1') {
